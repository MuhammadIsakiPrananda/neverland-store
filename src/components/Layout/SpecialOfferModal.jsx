...
const SpecialOfferModal = ({ isOpen, onClose, offers = [] }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-2xl mx-4 rounded-2xl shadow-2xl border border-slate-700/50 bg-gradient-to-br from-slate-900/95 via-slate-800/95 to-slate-950/95 backdrop-blur-xl overflow-hidden animate-fade-in-up">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 p-2 rounded-full bg-slate-800/50 hover:bg-red-500/80 text-white hover:text-white transition-all shadow-lg border border-slate-700/50"
          aria-label="Close"
        >
          ...
          <div className="w-full max-h-[60vh] overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-6 custom-scrollbar">
            ...
            {offers.map((promo, idx) => (
              <div key={idx} className="relative flex flex-col bg-slate-800/50 border border-slate-700/50 rounded-xl shadow-lg p-5 group hover:scale-[1.03] hover:shadow-sky-900/20 transition-all hover:border-sky-500/30">
                ...
                {/* Promo Description */}
                <p className="text-slate-300 text-sm mb-2 line-clamp-3 min-h-[48px]">{promo.description}</p>
                {/* Promo Code */}
                {promo.code && (
        ...
        <div className="absolute bottom-0 right-0 w-32 h-32 bg-gradient-to-tr from-indigo-500/20 via-sky-500/20 to-transparent rounded-full blur-2xl opacity-60 pointer-events-none" />
      </div>
    </div>
  );
};
